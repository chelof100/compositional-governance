...
%  Paper 4 of 4 — Agent Governance Series
% =============================================================================
...
Paper~3 studies the fairness of access to the governance boundary. Three
allocation mechanisms are analyzed:
\begin{itemize}[noitemsep]
  \item \textbf{M1} (Per-agent quota): fixed token budget per agent.
  \item \textbf{M2} (First-come-first-served): admission in arrival order.
  \item \textbf{M3} (Proportional lottery): randomized allocation by share.
\end{itemize}
Key impossibility results include:
\begin{itemize}[noitemsep]
  \item \textbf{Sybil Amplification} (Thm~5.1): under any identity-oblivious
    mechanism, an adversary controlling $k$ identities can amplify resource
  ...
  \item \textbf{Strategy-Proofness Impossibility} (Thm~5.4): no mechanism
    ...
    Sybil robustness under population heterogeneity.
  \item \textbf{BAR-Monitor Interference}: under M3, Sybil attacks depress
    the BAR signal, masking enforcement degradation from the IML monitor.
\end{itemize}
...
Table~\ref{tab:notation} collects the notation unified across the four papers.

\begin{table}[ht]
\centering\small
\caption{Unified notation for the four-paper series.}
\label{tab:notation}
\begin{tabular}{lp{5.8cm}l}
\toprule
\textbf{Symbol} & \textbf{Meaning} & \textbf{Source} \\
\midrule
$\Dec = \{\allow,\escalate,\deny\}$ & Decision domain & Papers~0--4 \\
...
\begin{enumerate}[label=(P\arabic*),noitemsep]
  ...
  \item \emph{Exact actor-level fairness under population heterogeneity}:
    ...
    heterogeneous population $\Agents$ (Paper~3 \cite{fairgov26},
    mechanisms M1/M2/M3).
